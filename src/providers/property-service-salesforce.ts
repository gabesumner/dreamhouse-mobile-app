...
export class PropertyService {
    ...
    findById(id) {
        return this.force.retrieve('Property__c', id,
            `id,
                                title__c,
                                address__c,
                                city__c,
    ...
    getFavorites() {
        return this.force.query(`SELECT id,
                            ...
                            FROM favorite__c
                            WHERE user__c='${this.force.getUserId()}'`)
            .then(response => response.records.map(favorite => this.prettifyFavorite(favorite)));
    }

    favorite(property) {
        return this.force.create('Favorite__c', {User__c: this.force.getUserId(), Property__c: property.id});
    }

    unfavorite(favorite) {
        return this.force.del('Favorite__c', favorite.id);
    }
